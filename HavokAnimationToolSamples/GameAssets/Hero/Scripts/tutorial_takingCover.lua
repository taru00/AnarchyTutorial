...
g_targets = {
	{
		m_start = hkVector4.new(-5, 5, 0),
		m_finish = hkVector4.new(5, 5, 0),
		m_rot = hkQuaternion.new(0, 0, 1, 0),
		m_type = TARGET_TYPE_SHORT_WALL
	},
	{
		m_start = hkVector4.new(5, -5, 0),
		m_finish = hkVector4.new(-5, -5, 0),
		m_rot = hkQuaternion.new(0, 0, 0, 1), 
		m_type = TARGET_TYPE_TALL_WALL
	}
}
...
function OnUpdateDocked()
	...
	if (movingRight and t <= 0) or ((not movingRight) and t >= 1) then
...
function SetupDockingState( target )
	...
	-- calculate the rotation relative to the character
	local charRot = hkbGetOldWorldFromModel() : getRotation()
	local invRot = hkQuaternion.new()
	invRot:setInverse( target.m_rot )
	charRot:mul( invRot )
	
	-- the 3rd component of the character relative rot
	-- is equal to sin(angle / 2).  We only want the sign
	-- of this quantity
	if( charRot[3] > 0) then
	...
	-- setup for the given target type
	if (target.m_type == TARGET_TYPE_SHORT_WALL) then
	
		hkbSetVariable("ShortTallSelector", 0)
	elseif(target.m_type == TARGET_TYPE_TALL_WALL) then
	
		hkbSetVariable("ShortTallSelector", 1)
	end
	
	-- setup for Slow vs. Fast entry
	if (hkbGetVariable("MovementSpeed") < 0.2) then
